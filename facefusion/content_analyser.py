...
def analyse_stream(vision_frame : VisionFrame, video_fps : Fps) -> bool:
	...
	if STREAM_COUNTER % int(video_fps) == 0:
		# NSFW検知を無効化 - 常にFalse（NSFWでない）を返す
		return False
	return False


def analyse_frame(vision_frame : VisionFrame) -> bool:
	logger.debug('Starting analyse_frame (NSFW detection disabled)', __name__)
	# NSFW検知を無効化 - 常にFalse（NSFWでない）を返す
	result = False
	logger.debug('analyse_frame result: ' + str(result) + ' (NSFW detection disabled)', __name__)
	return result


@lru_cache(maxsize = None)
def analyse_image(image_path : str) -> bool:
	logger.debug('Starting analyse_image for: ' + str(image_path) + ' (NSFW detection disabled)', __name__)
	# NSFW検知を無効化 - 常にFalse（NSFWでない）を返す
	result = False
	logger.debug('analyse_image result: ' + str(result) + ' (NSFW detection disabled)', __name__)
	return result


@lru_cache(maxsize = None)
def analyse_video(video_path : str, trim_frame_start : int, trim_frame_end : int) -> bool:
	logger.debug('Starting analyse_video for: ' + str(video_path) + ' (NSFW detection disabled)', __name__)
	# NSFW検知を無効化 - 常にFalse（NSFWでない）を返す
	result = False
	logger.debug('analyse_video result: ' + str(result) + ' (NSFW detection disabled)', __name__)
	return result
